  ...
  while ($archivo = readdir($directorio)){
   ...
   if (!file_exists('../'.$ruta.'/mini/'.$archivo)) {creador($archivo, $ruta, $carpeta);} /*enviamos el archivo despues de validar*/
     ...
     if ($pos === false) {
          ...
          $tarjeta.='<div class="botov"><p class="card-text precio">RD$ '.substr($a[1], 0, strpos($a[1],'.')).' </p> <button type="button" class="btn btn-primary" id="'.$modalid.$globo.'"  alt="'.$nombre.'" alt2="'.substr($a[1], 0, strpos($a[1],'.')).'"  alt3="'.$ruta . "/mini/" . $archivo.'"  onclick="imagen('.$idj.')">';
          $tarjeta.='<i class="fa fa-plus" aria-hidden="true"></i></button></div><p class="card-text"><small class="text-muted">(No incluye impuestos)</small></p>';
          $tarjeta.='</div></div></div></div>';
          $tarjeta.='<div class="modal fade" id="'.$modalid.'Modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true"><div class="modal-dialog modal-xl modal-dialog-centered"><div class="modal-content">';
