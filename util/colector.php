  ...
  while ($archivo = readdir($directorio)){
   ...
   if (!file_exists('../'.$ruta.'/mini/'.$archivo)) {creador($archivo, $ruta, $carpeta);} /*enviamos el archivo despues de validar*/
     ...
     if ($pos === false) {
          ...
          $tarjeta.='<div class="botov"><p class="card-text" style="font-weight: 700; font-size: 1.8em; color: blue; margin: 10px;">RD$ '.substr($a[1], 0, strpos($a[1],'.')).' </p><div type="button" class="btn btn-primary" id="cuenta'.$globo.'" alt="'.$nombre.'" alt2="'.substr($a[1], 0, strpos($a[1],'.')).'" onclick="imagen('.$globo.')">Agrega a tu Menu<i class="fa fa-share" aria-hidden="true"></i></div></div><p class="card-text"><small class="text-muted">(No incluye impuestos)</small></p>';
          $tarjeta.='</div></div></div></div>';
          $tarjeta.='<div class="modal fade" id="'.$modalid.'Modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true"><div class="modal-dialog modal-xl modal-dialog-centered"><div class="modal-content">';
